...
class DesktopWorkspaceScrollTest {
    ...
    @Test fun breadcrumbsReturnToAncestorsWithoutReplayingCommandsAndKeepDirtyGuard() {
        ...
        val elevation = courses.enter(DesktopNavigation.itemById(root.workflow, "setup.controls.elevation-cache")!!)
        val trail = DesktopNavigation.breadcrumbStates(elevation)
        assertEquals(listOf("Setup", "Courses", "Elevation Data"), trail.map { it.first })
