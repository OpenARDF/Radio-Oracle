...
class SportIdentSi8OwnerNamePlannerTest {
    ...
    fun encodedPreviewRoundTripsThroughExistingSi8Reader() {
        ...
        assertEquals(bytes.size, preview.byteCount)
        assertEquals(card.siNumber, preview.siNumber)
    }

    @Test
    fun countsSeparatorsAndRejectsOverflowWithoutTruncating() {
        val exact = SportIdentSi8OwnerNamePlanner.preview(card, "A".repeat(11), "B".repeat(11))
        assertEquals(24, exact.byteCount)
        assertNotNull(exact.encodedOwnerText)
        val overflow = SportIdentSi8OwnerNamePlanner.preview(card, "A".repeat(12), "B".repeat(11))
        assertEquals(25, overflow.byteCount)
