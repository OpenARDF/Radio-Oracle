...
Section 1 also checks the stored route against the applicable USA rules. Classic and Foxoring checks compare the route's effective length, or horizontal fallback when elevation is incomplete, with the category length range and compare assigned fox count with the category requirement. Sprint checks compare assigned fox count with twice the Classic category assignment and compare estimated ideal time with the approximately 15-minute target. Spacing checks use straight-line distance between known protected point coordinates: Classic foxes and the finish beacon must be at least 750 m from the start, 400 m from the finish beacon, and 400 m from other foxes; Youth Classic uses the 500 m start-distance rule from Appendix Y. Sprint checks the 100 m transmitter spacing and the 100 m fox-from-start rule, while allowing the beacon and spectator to be less than 100 m from the start. Foxoring checks the 250 m transmitter spacing and 250 m start-distance rule. Minimum spacing rows check all applicable points or transmitter pairs, then report the nearest checked point or closest pair as the limiting result. Rule-check rows put the measured result first and the applicable rule in parentheses. Failed rule checks are shown as red violation text.
...
Section 3 summarizes the results. When both a stored route and calculated candidate exist, it compares their order, distance metrics, estimated time, wait behavior, USA rules checks, elevation profiles, and compact 2D route depictions. When no stored route exists, it summarizes only the calculated candidate.
