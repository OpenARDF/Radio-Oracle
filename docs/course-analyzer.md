...
The `Apply Calculated Route` button applies the calculated ideal route to the open Event File. It replaces the selected category's protected ideal order, re-encrypts the protected course data with that ideal order, and updates the affected fox public labels to the calculated numbering scheme. The change is saved in memory as an unsaved Event File change; save the Event File to persist it to disk.

The `Apply Fox Renumbering Only` button applies only the Section 1 wait-time renumbering recommendation when it improves the stored route's Classic wait estimate. It updates shared fox public labels and re-encrypts affected protected course labels and protected ideal-order text across categories, without replacing the stored route geometry. The button is disabled when no improved renumbering was calculated.
