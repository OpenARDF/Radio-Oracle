...
# EXPERIMENTAL: programs one SI-Card8, then requires removal and fresh insertion to verify.
...
# EXPERIMENTAL: opt-in seven-word trial with a 200 ms post-reply pause; requires exact hardware approval.
sportident-owner-native-seven-word-paced-trial request:
    JAVA_HOME="{{java_home}}" ./scripts/gradle-sequential.sh :desktopApp:desktopSportIdentNativeOwnerSevenWordPacedTrial {{quote("-PsiOwnerWriteRequest=" + request)}}

# EXPERIMENTAL: opt-in six-word trial with a 200 ms post-reply pause; requires exact hardware approval.
...
# EXPERIMENTAL: stops after three acknowledged owner words, before read-back; requires recovery assessment.
sportident-owner-native-stop-after-three request:
    JAVA_HOME="{{java_home}}" ./scripts/gradle-sequential.sh :desktopApp:desktopSportIdentNativeOwnerStopAfterThree {{quote("-PsiOwnerWriteRequest=" + request)}}
